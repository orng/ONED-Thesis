...
\begin{theorem}
  Given that we have previously processed bags $B_{1}...B_{n-1}$ the minimal-new sets of maximum size $h$ can be enumerated from sub-bag $S_{n, j}\in B_{n}$ in \BigO{p|S_{j}|2^{h}/h!}, where $p$ is the maximum amount of sub-bags in any bag $B_{i} \mid i \in \mathbb{N}, i \in [1,n-1]$.
\end{theorem}
\begin{proof}
